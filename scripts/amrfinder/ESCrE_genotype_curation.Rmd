...
amrfinder <- readRDS("./data/amrfinder/amrfinder_cleaned.txt")  %>% subset(genomic_id %in% isolates)
amrfinder_mat <- readRDS("./data/amrfinder/amrfinder_matrix.RDS") %>% subset(rownames(.) %in% isolates)
```

```{r}
blactam_info <-  amrfinder %>% subset(class == "BETA-LACTAM") %>% select(element_symbol,element_name,class,subclass) %>% distinct
paste0("Detected b-lactamase genes: ")
sort(blactam_info$element_symbol)

paste0("Detected ESBL genes: ")
ESBL_genes <- amrfinder[grepl("extended-spectrum",amrfinder$element_name),'element_symbol'] %>% unique %>% sort
sort(ESBL_genes)

paste0("Summary information about these beta-lactamases")

# Presence in isolates
blactam_info$count <- sapply(blactam_info$element_symbol, FUN=function(x){
  amrfinder_mat[[x]] %>% sum
})
...
# ESBL 
blactam_info$ESBL <- ifelse(blactam_info$element_symbol %in% ESBL_genes,TRUE,FALSE)

# Carbapenemase
BLA_carbapenem_genes <- amrfinder %>% subset(class =="BETA-LACTAM" & subclass=="CARBAPENEM") %>% .$element_symbol %>% unique %>% sort 
blactam_info$carbapenemase <- ifelse(blactam_info$element_symbol %in% BLA_carbapenem_genes,T,F)
...
blactam_info %>% subset(ESCrE ==T) %>% .$element_symbol %>% unique %>% sort

paste0("Overall data:")

blactam_info %>% select(-class) %>% arrange(-count) %>% gontjesr::favorite_kable()

# Save
blactam_info <- blactam_info %>% arrange(element_symbol)

saveRDS(blactam_info,"./data/amrfinder/beta_lactamase_summary_index_cohort.RDS")
```