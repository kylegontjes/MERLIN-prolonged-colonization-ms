...
# Get ESCRE qc-passing genome data
```{r}
...
paste0("Number of unique QC passing duplicates eduplicates: ", length(metadata_duplicates$genomic_id_qc_pass_escre %>% unique))
```
...
MERLIN_23_plate <- sequencing_MERLIN_plates %>% subset(Genome.ID == "MERLIN_23") %>% .$Sequencing.plate %>% str_split(.,",",simplify=T) %>% .[,1] %>% gsub("Sequencing plate MERLIN ","",.)
...
metadata_MERLIN_211_MERLIN_246<- subset(metadata,genomic_id_qc_pass %in% duplicates[10])  
metadata_split_MERLIN_211_MERLIN_246 <- separate_rows(metadata_MERLIN_211_MERLIN_246,genomic_id,sep = ';')  
metadata <- subset(metadata,!metadata$genomic_id_qc_pass %in% duplicates[10]) %>% bind_rows(.,metadata_split_MERLIN_211_MERLIN_246)
```

# 11. MERLIN_234 MERLIN_233 
## Story: They are close enough - 25 SNP different different! 
## Choice: Keep 234
```{r}
