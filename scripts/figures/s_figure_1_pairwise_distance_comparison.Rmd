...
update_pair_types_df_w_questions <- function(pair_types_df){
  ...
  same_individual <- ifelse(pair_types_df$MERLIN_subject_id1==pair_types_df$MERLIN_subject_id2,T,F)
  ...
  dataset <- cbind(pair_types_df,index_cases=index_cases,same_household=same_household,same_household_visit=same_household_visit,ST_pair=ST_pair,same_specimen_category=same_specimen_category,same_individual=same_individual,both_human=both_human,both_pet=both_pet,both_env=both_env)
  return(dataset)
}
```
...
overall_pair_types_index <- subset(overall_pair_types,MERLIN_subject_id1 %in%  df_index$MERLIN_subject_id & MERLIN_subject_id2 %in% df_index$MERLIN_subject_id )
...
s_figure_1 <- s_figure_1 +  annotation_custom(ggplotGrob(s_figure_1_inlet), xmin = 65000, xmax = 155000, ymin = 150, ymax = 800)
