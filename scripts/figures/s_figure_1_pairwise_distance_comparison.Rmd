...
```{r,fig.width=8.25,fig.height=5.5}
...
s_figure_1_inlet <- ggplot(data=overall_pair_types_index %>% subset(pairwise_dist <500),aes(x=pairwise_dist,fill=same_household)) + geom_histogram(bins=100) + theme_bw_me + xlab("") + ylab("") + pairtype_fill + geom_vline(xintercept = 25,colour = 'red',linetype = 2) + s_figure_1_inset_format 
...
```{r}
ggsave(plot = s_figure_1,filename = './figures/s_figure_1_pairwise_SNP_distance.png', dpi = 900, width = 8.25, height = 5.5,bg='white')
```