---
title: "Supplemental Figure 3: Accuracy of species-level evaluation of prolonged colonization"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
# Generate data
```{r,fig.height=9.75,fig.width=7.8}
outcomes <- c('index_at_1_and_or_2_Species_ST','index_at_1_and_or_2_Species_ST_cluster',"index_at_1_and_or_2_Species_ST_cluster_ESCrE_genes_plasmid_linked")
...
comparison_of_any_ESCRE_to_WGS_outcomes$outcome_rename <- recode(comparison_of_any_ESCRE_to_WGS_outcomes$outcome,'index_at_1_and_or_2_WGS_escre'='Any ESCrE','index_at_1_and_or_2_Species'='Species','index_at_1_and_or_2_Species_ST'='Same sequence type','index_at_1_and_or_2_Species_ST_cluster'='Same strain','index_at_1_and_or_2_ESCrE_genes'='ESCrE gene','index_at_1_and_or_2_ESCrE_plasmidic_genes'= 'Plasmidic ESCrE gene','index_at_1_and_or_2_ESCrE_plasmids'= 'ESCrE plasmid',
  'index_at_1_and_or_2_Species_ST_cluster_ESCrE_genes'='Strain + ESCrE gene',
  'index_at_1_and_or_2_Species_ST_cluster_ESCrE_genes_plasmid_linked'='Same strain + plasmid-linked ESCrE gene') %>% factor(.,levels = c("Same sequence type","Same strain","Same strain + plasmid-linked ESCrE gene"))
...
A <- ggplot(data=comparison_of_any_ESCRE_to_WGS_outcomes,aes(x=outcome_rename,y=spec)) + geom_bar(stat='identity',fill="black") + ylab("Specificity (%)") + xlab(NULL)+ theme_bw_me+ ylim(NA,100.1)  + s_figure_2_format  +   geom_text(aes(label=paste0(format(spec, nsmall = 1), "%")), vjust=0,nudge_y=1,size=3.25) 
B <- ggplot(data=comparison_of_any_ESCRE_to_WGS_outcomes,aes(x=outcome_rename,y=precision)) + geom_bar(stat='identity',fill="black") + ylab("Positive predictive value (%)") + xlab(NULL) + theme_bw_me+ ylim(NA,100.1)  + s_figure_2_format  +  geom_text(aes(label=paste0(format(precision, nsmall = 1), "%")), vjust=0,nudge_y=1,size=3.25)  
C<- ggplot(data=comparison_of_any_ESCRE_to_WGS_outcomes,aes(x=outcome_rename,y=accuracy)) + geom_bar(stat='identity',fill="black") + ylab("Accuracy (%)") + xlab(NULL) + theme_bw_me+ ylim(NA,100.1) + s_figure_2_format + geom_text(aes(label=paste0(format(accuracy, nsmall = 1), "%")), vjust=0,nudge_y=1,size=3.25)  

s_figure_3 <- cowplot::plot_grid(A,B,C,labels = "AUTO",ncol=3)
s_figure_3
```

# Save figure
```{r}
ggsave(plot = s_figure_3,filename = './figures/s_figure_3_diagnostic_statistics_species_prolonged_colonization.png',dpi = 900, width = 6, height = 6, bg='white')
```
