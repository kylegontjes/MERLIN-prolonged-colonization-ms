...
# Data
```{r cars} 
ESCrE_genotype_data <- readRDS("./data/ESBL_plasmids/MERLIN_ESCrE_proportion_plasmidic.RDS")
ESCRE_genotype_info <- readRDS("./data/amrfinder/beta_lactamase_summary_index_cohort.RDS")
```

```{r}
ESCrE_genotype_data <- left_join(ESCrE_genotype_data %>% mutate(element_symbol = ESCrE),ESCRE_genotype_info %>% select(element_symbol,blactam_class,ESBL,carbapenemase,ampC_like))
...
ESBL_prop_data <- ESCrE_genotype_data %>% select(element_symbol,category,n_chromosome,n_plasmid,prop_plasmid) %>% arrange(-prop_plasmid,-n_plasmid)
